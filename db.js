...
// Funciones CRUD genéricas — versión mejorada
const DB = {
  // GET
  get: (table) => {
  ...
  // ✅ CREATE — versión corregida con IDs secuenciales tipo #1, #2, #3...
  create: (table, item) => {
    const data = DB.get(table);
    let newId = 1;

    if (data.length > 0) {
      const idsNumericos = data
        .map(i => parseInt(String(i.id).replace('#', '')))
        .filter(n => !isNaN(n));
  ...
  update: (table, id, updates) => {
    ...
    console.warn(`[DB.update] No se encontró el id "${id}" en la tabla "${table}".`);
    return null;
  },
  ...
  debug: () => {
    ...
    [
      'solicitudes', 'clientes', 'proyectos', 'inventario',
      'cotizaciones', 'cuentasCobro', 'personal', 'pagos', 'auditoria'
    ].forEach(table => {
      const data = DB.get(table);
      console.log(`${table}:`, data.length, 'registros', data);
    });
  }
};
...
if (typeof window !== 'undefined') {
  initDB();
  window.DB = DB;
  console.log('Base de datos inicializada. Usa DB.debug() o DB.reset().');
}
